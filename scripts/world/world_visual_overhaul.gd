...
@export var seed_value := 20260816
@export var grass_count := 1850
@export var tree_count := 44
...
const COTTAGES := [
    [Vector3(-10.5, 0.0, -7.5), 18.0, 0],
    [Vector3(10.8, 0.0, -10.0), -24.0, 1],
    [Vector3(-9.0, 0.0, -23.0), 13.0, 2],
    [Vector3(11.5, 0.0, -25.5), -15.0, 0]
]
...
func _material(key: String, color: Color, roughness := 0.92) -> StandardMaterial3D:
...
func _emissive_material(key: String, color: Color, energy := 1.35) -> StandardMaterial3D:
    if materials.has(key):
        return materials[key]
    var mat := StandardMaterial3D.new()
    mat.albedo_color = color
...
func _mesh(mesh: Mesh, material: Material, position: Vector3, parent: Node3D = null, scale_value := Vector3.ONE, rotation_value := Vector3.ZERO) -> MeshInstance3D:
...
func _build_grass_field() -> void:
    var grass_mesh := _make_grass_tuft_mesh()
    ...
    while placed < grass_count and attempts < grass_count * 10:
        attempts += 1
        var x := rng.randf_range(-78.0, 78.0)
        var z := rng.randf_range(-78.0, 78.0)
        var p := Vector3(x, 0.035, z)
        if not _grass_allowed(p):
            continue
        var yaw := rng.randf_range(0.0, TAU)
        var width_scale := rng.randf_range(0.75, 1.35)
        var height_scale := rng.randf_range(0.65, 1.55)
        ...
        var tint := rng.randf_range(-0.035, 0.055)
...
func _grass_allowed(p: Vector3) -> bool:
    ...
    for cottage_data in COTTAGES:
        var cottage_position: Vector3 = cottage_data[0]
...
func _build_path(start: Vector3, finish: Vector3, pieces: int, material: Material) -> void:
    for i in range(pieces):
        var t := float(i) / float(maxi(1, pieces - 1))
        ...
        _mesh(patch, material, p, self, Vector3(rng.randf_range(0.95, 1.35), 1.0, rng.randf_range(0.72, 1.08)), Vector3(0.0, rng.randf_range(0.0, TAU), 0.0))

func _build_forest() -> void:
    var centers := [
        Vector3(-43.0, 0.0, 18.0), Vector3(42.0, 0.0, 20.0),
        Vector3(-49.0, 0.0, -18.0), Vector3(50.0, 0.0, -20.0),
        Vector3(-38.0, 0.0, -52.0), Vector3(37.0, 0.0, -54.0),
        Vector3(0.0, 0.0, -62.0)
    ]
    var per_cluster := maxi(4, tree_count / centers.size())
    var spawned := 0
    for center in centers:
        for _i in range(per_cluster):
            ...
            var distance := rng.randf_range(2.0, 12.0)
            var p := center + Vector3(cos(angle) * distance, 0.0, sin(angle) * distance)
...
func _spawn_tree(position: Vector3, scale_value: float, palette_index: int) -> void:
    ...
    var foliage_colors := [Color("#3f7f42"), Color("#4f934b"), Color("#356d3d")]
    ...
    for i in range(crown_positions.size()):
        var scale_variation := 0.88 + float((i * 7 + palette_index) % 5) * 0.055
        _mesh(crown_mesh, foliage, crown_positions[i], root, Vector3(1.15 * scale_variation, 0.92 * scale_variation, 1.08 * scale_variation))
...
func _build_cottages() -> void:
    for data in COTTAGES:
        _build_cottage(data[0], deg_to_rad(float(data[1])), int(data[2]))
...
func _build_cottage(position: Vector3, yaw: float, palette_index: int) -> void:
    ...
    var plaster_colors := [Color("#d7c7a4"), Color("#c9d0b4"), Color("#d4b7a0")]
    var roof_colors := [Color("#364b55"), Color("#4a3c43"), Color("#48513b")]
    ...
    for p in [Vector3(-2.84, 1.78, 2.06), Vector3(2.84, 1.78, 2.06), Vector3(-2.84, 1.78, -2.06), Vector3(2.84, 1.78, -2.06)]:
        _mesh(post, timber, p, root)
    ...
    for x in [-1.85, 1.85]:
        _mesh(frame_v, timber, Vector3(x, 1.8, 2.32), root)
        _mesh(frame_h, timber, Vector3(x, 1.8, 2.32), root)
...
func _build_rocks_and_shrubs() -> void:
    ...
    for i in range(28):
        var angle := rng.randf_range(0.0, TAU)
        var distance := rng.randf_range(15.0, 70.0)
        var p := Vector3(cos(angle) * distance, 0.0, sin(angle) * distance)
        if i % 2 == 0:
            _mesh(rock_mesh, rock_mat, p + Vector3(0.0, 0.35, 0.0), self, Vector3(rng.randf_range(0.7, 1.5), rng.randf_range(0.5, 0.9), rng.randf_range(0.8, 1.45)), Vector3(rng.randf_range(-0.18, 0.18), rng.randf_range(0.0, TAU), rng.randf_range(-0.18, 0.18)))
        else:
            _mesh(shrub_mesh, shrub_mat, p + Vector3(0.0, 0.42, 0.0), self, Vector3(rng.randf_range(0.9, 1.5), rng.randf_range(0.7, 1.1), rng.randf_range(0.9, 1.5)))

func _spawn_two_villagers() -> void:
    _spawn_villager(NPC_MAGE, "Elena", "herborista", "El bosque está más vivo de lo que parece. Recoge solo lo que necesites.", Vector3(-3.3, 0.08, -8.0), 2.8)
    _spawn_villager(NPC_RANGER, "Gael", "cazador", "Los árboles grandes dan buena madera. Lleva el hacha equipada.", Vector3(3.6, 0.08, -17.0), 3.0)
