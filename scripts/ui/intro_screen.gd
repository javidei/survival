...
const COMMODORE_FONT_SOURCE: FontFile = preload("res://assets/fonts/Commodore Pixelized v1.2.ttf")
...
func _ready() -> void:
    ...
    commodore_font = _pixel_font(COMMODORE_FONT_SOURCE)
    queue_redraw()
...
func _draw() -> void:
    ...
    _center_text(commodore_font, "BIENVENIDO A", left, content_width, 150.0 * scale_factor, maxi(12, int(round(28.0 * scale_factor))), COL_TOP)
    ...
    _center_text(commodore_font, "NARANJAL SURVIVAL - PROTOTIPO 0.2.6", left, content_width, 665.0 * scale_factor, maxi(10, int(round(18.0 * scale_factor))), COL_VERSION)
